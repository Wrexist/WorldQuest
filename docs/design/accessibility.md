...
| Problem | Approach |
|---|---|
| **Map tap questions with a screen reader** | Equivalent accessible list of candidates; same item, same FSRS state. **Done — 65 of 65.** `tpl.country-to-map.mc4` ships with the sibling `tpl.location-of.mc4` — "Where in the world is Sweden?", same attribute, same `user_facts` row — and `composeLesson` swaps it in whenever a screen reader is on. It built for only 35 of 65 countries until the distractor fallback was fixed: the sibling drew wrong answers from other subregions of the same region, and South America contains exactly one subregion, so all three distractors read "South America", deduplicated into the correct option, and the question was dropped. The fallback never fired because it triggered on an empty candidate pool and the pool was not empty — it just could not produce four different strings. It now falls back on too few OPTIONS, to `other-values`: any subregion at all, which is what the question was always asking. `thesis.test.ts` asserts every fact can be *asked* without sight, not merely that a safe item exists for it — the weaker assertion is what let 30 missing countries look green. |
