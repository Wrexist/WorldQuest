...
| `progress/globe` | A globe on the "Your world" card | **Drawn 2026-08-09.** Higgsfield credits were exhausted, so it was generated via ChatGPT's web image tool against the exact `asset-prompts.md §10` prompt instead, then background-removed and cropped locally. Committed to `docs/design/assets/rewards/globe.png`; `pnpm build:art` still needs to be run to derive the in-app variants and wire it into the card. |
| §8b continent silhouettes | A landmark behind each continent tile | **Drawn 2026-08-09, six of seven.** The subject-choice blocker was resolved by picking generic, unencumbered landforms per continent (alpine peaks, terraced ridge, acacia and savanna, mesa buttes, andean ridge, coral atoll — see `asset-prompts.md §8b`) instead of the reference's restricted landmarks. All six generated via ChatGPT, thresholded to transparent white-on-nothing, committed to `docs/design/assets/continents-silhouette/`. Antarctica stays undrawn — no card exists for it yet. Not yet wired into the Explore tiles or run through `pnpm build:art`. |
...
| Today's Quest card as Home's hero, with the green `Continue` and Atlas as its scene (screen 3) | **The first-week course path is the hero**: unit banners and a zig-zag of steps, the current one lit green with a `Start` callout. The quest card sits below the path with a blue `Start quest`, and Atlas moved from beside the greeting into the current unit's banner | The launch brief makes one course and "one green primary recommendation" the product, with everything else secondary (L08/U05). A quest card above the path pushed the primary action below the fold at 320 pt (U01), and one inside it would split the trail. One mascot per screen, beside the part of the path you are on. The graft is recorded in `.claude/skills/dna-transplant/references/duolingo-worldquest.md` |
