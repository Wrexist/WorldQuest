...
`escape-hatches`, `reachability`, `five-states` and `scrollable`.
`pnpm design:shots` also runs an **`en-XA` pseudo-locale pass** — 6 routes × 3 viewports
with every string inflated ~40 % — which had never been possible before: the harness drives
the exported bundle, so `enablePseudoLocale()`'s `isDev()` gate refused and returned false
silently, and the function had no caller outside its own unit test. It is clean: no sideways
scroll, no clipping, and the only untranslated text is currency and country names, which are
content-pack facts rather than copy.
...
| Server-authoritative rewards | ✅ **Real for every reward the balance table funds, as of 2026-08-18.** The box says "client cannot forge it", which is a stronger claim than "nothing on the client writes a balance" and was not true. `user_facts.mastery` was never written by anything, so the count on Home was always zero; `streaks` was written only by the signup trigger, so the streak was always zero; the shop performed no spend, so every cosmetic was free; and `answeredAt` was taken from the client unvalidated, which minted mastery and `factMastered` XP. All four are closed — see `record_lesson`, the mastery trigger, `purchase_item` and `_shared/submission-time.ts`. Since then: streak milestones are paid, `lessons.hearts_lost` is written, the freeze is buyable, `expire_streaks()` records a break hourly per user timezone, and every achievement event has a producer — seven of twelve could not move at all. **2026-08-18 closed four more.** The daily QUEST is now paid: the device composes it (it must — a quest has to work on a plane), the first submission of a local day pins it server-side, and `record_lesson` pays per slot from evidence in `review_log` and `lessons`, under the same advisory lock as the insert. `continue_lesson` charges the 250 coins the out-of-hearts button had printed and never taken. `repair_streak` is the endpoint the repair card's permanently-disabled button was waiting for. And the lesson kind is validated before it reaches a Postgres enum, so a bad one parks with a 400 instead of burning five retries on a 500. **And achievements, the last one.** `submit-lesson` evaluates the catalogue with the same `evaluateAll` the device runs — vendored, byte-identical, asserted by a bundle guard — and `record_lesson` banks the tier in the same transaction as the lesson that earned it. `achievement_unlocks` is keyed on (user, achievement, tier), so a replay pays nothing. Two rules changed on the way: `ach.explorer.continents` counted continent PAGES OPENED, which a server cannot see and six taps completed — it counts regions answered correctly in now, which is what its copy always said — and `ach.level.climber` had no producer at all, because nothing ever put a `level` in an event payload. What is left unpaid in the table is three rewards whose PRODUCER does not exist: no referral system, no collection-complete event, no weekly league settlement. That is a different kind of gap from the four that turned out to be forgotten wiring. Sign-in exists, and so does sign-out — which used to wipe the offline queue in silence and now names what would be lost before the control rather than after it. |
| The day boundary | ✅ **Real as of 2026-08-18, and it was not before.** `profiles.timezone` decides the streak day, the XP soft-cap window, `isFirstLessonOfDay` and which rows `expire_streaks()` breaks each hour. It defaults to `'UTC'`, `signInAnonymously()` sends no metadata, and no screen had ever written a profile row — so every user this product created rolled over at UTC midnight. In Auckland that ends the day at 11 a.m.: an evening lesson counts towards tomorrow and a streak can break while the user still considers the day young. `lib/timezone.ts` writes it on mount and on resume. Every piece of machinery for local days was built, tested and documented at length; the line supplying its input was missing, and nothing could have caught that except asking who writes the column. |
