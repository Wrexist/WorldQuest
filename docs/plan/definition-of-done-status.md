...
| Server-authoritative rewards | 🟡 **Real for XP, coins, mastery, streaks, the daily quest and all three purchases as of 2026-08-18; not yet for achievements.** The box says "client cannot forge it", which is a stronger claim than "nothing on the client writes a balance" and was not true. `user_facts.mastery` was never written by anything, so the count on Home was always zero; `streaks` was written only by the signup trigger, so the streak was always zero; the shop performed no spend, so every cosmetic was free; and `answeredAt` was taken from the client unvalidated, which minted mastery and `factMastered` XP. All four are closed — see `record_lesson`, the mastery trigger, `purchase_item` and `_shared/submission-time.ts`. Since then: streak milestones are paid, `lessons.hearts_lost` is written, the freeze is buyable, `expire_streaks()` records a break hourly per user timezone, and every achievement event has a producer — seven of twelve could not move at all. **2026-08-18 closed four more.** The daily QUEST is now paid: the device composes it (it must — a quest has to work on a plane), the first submission of a local day pins it server-side, and `record_lesson` pays per slot from evidence in `review_log` and `lessons`, under the same advisory lock as the insert. `continue_lesson` charges the 250 coins the out-of-hearts button had printed and never taken. `repair_streak` is the endpoint the repair card's permanently-disabled button was waiting for. And the lesson kind is validated before it reaches a Postgres enum, so a bad one parks with a 400 instead of burning five retries on a 500. What remains: achievement AWARDS. The unlock is real, visible and evaluated from server-derived events, but the XP and coins behind a tier are paid by nothing, because the server holds no achievement state — see `balance.ts` beside the numbers and `achievements.md §5`. Sign-in exists, and so does sign-out — which used to wipe the offline queue in silence and now names what would be lost before the control rather than after it. |
