...
| # | Work | Why now |
|---|---|---|
| 4.1 | **Currency facts** | ✅ 64 countries, `slow` volatility, full names ("Swedish krona", not "krona") as the sourced value, plus a `shortNames` form ("krona") that every QUESTION is built from — the demonym in the full name gave the answer away in both directions, in 54 of these 64 values (the other ten are the euro). See content-pipeline.md §2. Zimbabwe is absent: its currency changed twice in five years and the USD circulates — a fact already drifting is worse than a gap. **Language is deliberately still open** — Belgium, Switzerland, Canada, India and South Africa have several official languages, and picking one is exactly the unilateral call the content policy forbids. |
