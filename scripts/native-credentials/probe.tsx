...
async function run(): Promise<string> {
  ...
  if (stage === undefined) {
    ...
    await store.setItem(KEY, 'synthetic-B')
    proof.set('stage', 'restart')
    return 'PASS_MIGRATION_READY_RESTART'
  }
  if (stage === 'restart') {
    const old = createSessionStorage()
    check(await old.getItem(KEY) === 'synthetic-B', 'restart lost credential')
...
function Probe() {
  ...
  useEffect(() => { void run().then(setResult, () => setResult('FAIL_CREDENTIAL_PROOF')) }, [])
