...
const walk = dir => fs.readdirSync(dir, { withFileTypes: true }).filter(e => !['node_modules', '.git', 'coverage'].includes(e.name)).sort((a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0).flatMap(e =>
