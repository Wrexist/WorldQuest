...
const FUNCTIONS: Record<string, FunctionSpec> = {
  'submit-lesson': {
    ...
    shared: ['submission-time.ts', 'parse-submission.ts', 'achievement-events.ts'],
    generated: () => [
 ...
 *
 * This script is what VENDORS the engine into the deployable function, so it cannot
 * import the package it is flattening — a build step that resolves through the thing it
 * is producing is a resolution order nobody wants to debug at deploy time.
 * `packages/content/scripts/validate.ts` had a third copy and now calls the real
 * function; this is the last one, and it stays because of what this file is.
 *
 * Exported so the copy can be COMPARED to the original rather than merely labelled one.
 * `build.test.ts` runs both over the whole truth table, which the shipped packs cannot
 * do: all three of their non-quizzable facts carry `quizzable: false` AND
 * `sensitivity: "review-required"`, so dropping any single clause from this function
 * changes nothing about the current content and a packs-only comparison stays green.
 * That is exactly the drift worth catching — it goes unnoticed until the first fact that
 * relies on one clause alone, and by then the wrong thing has already shipped.
 */
export function isQuizzableCopy(fact: {
  quizzable?: boolean
  volatility?: string
  sensitivity?: string
}): boolean {
...
function buildAnswerKey(): string {
  ...
  /**
   * fact id → the attribute it DECLARES.
   *
   * Not derived from the id. `geo.AR.continent` declares `"attribute": "location"`, and
   * `ach.locations.collector` filters on `location`, so the four tiers of that
   * achievement were unreachable for as long as both sides split the id on dots. A fact
   * id is a permanent opaque key; its last segment is a coincidence, not a contract.
   */
  ...
  for (const file of readdirSync(packsDir)) {
    ...
    for (const fact of pack.items ?? []) {
      ...
      if (typeof fact.attribute === 'string') attributeByFact[fact.id] = fact.attribute

      if (isQuizzableCopy(fact)) (factsByEntity[fact.entity] ??= []).push(fact.id)
    }
  }
 ...
 * Generated by supabase/functions/build.ts — do not edit.
 *
 * fact id → the attribute the fact DECLARES, which is what an achievement rule's
 * \`where: { attribute }\` filter names. Read rather than derived: see buildAnswerKey().
 *
 * On one line, like the achievement catalogue and for the same reason: 350 entries of
 * generated lookup that nobody reads by hand, and the pack it comes from is the readable
 * copy.
 */
export const ATTRIBUTE_BY_FACT: Record<string, string> = ${JSON.stringify(attributeByFact)}
`
}
